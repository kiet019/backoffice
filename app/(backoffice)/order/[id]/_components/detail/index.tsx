...
import { getOrderStatusColor, getPlaceColor, getTagColor } from '@/app/(backoffice)/order/(view)/_components/config'

export const OrderDetailCard = ({ orderDetail }: { orderDetail: OrderDetail }) => {
  return (
    <FlexBetween marginRight={5} alignItems='baseline'>
      ...
      <div>
        ...
        <FieldData name='Khu vực'>
          <StyledChip
            size='small'
            label={orderDetail.place.name}
            color={'white'}
            backgroundColor={getPlaceColor(orderDetail.place.id)}
          />
        </FieldData>
        <FlexBox alignItems={'center'} marginTop={0.5}>
          <StyledChip
            size='small'
            label={orderDetail.tag[0].name}
            color={'white'}
            backgroundColor={getTagColor(orderDetail.tag[0].id)}
          />
          <Chip
            sx={{
              borderRadius: '6px',
      ...
      <div>
        <FieldData name='Ngày tạo đơn hàng'>
          <T fontWeight={500}>
            {orderDetail.createDate.date} {orderDetail.createDate.time}
          </T>
        </FieldData>
        <FieldData name='Ngày hoàn tất thanh toán'>
          <T fontWeight={500}>
            {orderDetail.paymentDate.date} {orderDetail.paymentDate.time}
          </T>
        </FieldData>
        <FieldData name='Hình thức thanh toán'>
          <T fontWeight={700} color='#2E7D32'>
